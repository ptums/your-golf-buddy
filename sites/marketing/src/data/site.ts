...
export const API_URL =
  import.meta.env.PUBLIC_API_URL || "https://api.yourbuddy.golf";
export const CONTACT_EMAIL = "hello@yourbuddy.golf";

/**
 * Feature flag for the paid "Golf Buddy Pass". OFF by default — the whole
 * paid surface (pricing, buy CTAs, pass FAQ, the /welcome flow, the Offer in
 * JSON-LD) stays hidden and the site reads as a free app. Flip on at launch by
 * setting PUBLIC_PASS_ENABLED=true in the marketing build env (see docs/pass.md).
 */
export const PASS_ENABLED = import.meta.env.PUBLIC_PASS_ENABLED === "true";

/** Stripe Payment Link — set PUBLIC_STRIPE_PAYMENT_LINK at launch. */
export const BUY_URL =
  import.meta.env.PUBLIC_STRIPE_PAYMENT_LINK || `${SITE_URL}/#pricing`;
...
/** FAQs shown when the paid Pass is live. */
export const passFaqs: { q: string; a: string }[] = [
  {
...
export const baseFaqs: { q: string; a: string }[] = [
  {
    q: "Is it really free?",
    a: "Yes. The full app — scoring, notes, history, swing tips, and practice drills — is free, with no ads and no account.",
  },
  {
  ...
  },
  {
    ...
    a: "On your device, in the browser's local storage. If you turn on cloud sync, a copy is also stored on our server — tied to a random key generated on your device, not to your name or email. You can delete all synced data from the app's settings at any time.",
  },
  {
    q: "How do I move to a new phone?",
    a: "The app gives you a recovery key you can save. Enter it on a new device to pull your rounds back.",
  },
  {
...
export const faqs: { q: string; a: string }[] = PASS_ENABLED
  ? [...baseFaqs, ...passFaqs]
  : baseFaqs;
